...
| P8 | ~~Vulkan shaders unoptimized~~ | ✅ Completed | F16 GEMM, q4_k_m dequant, elementwise (affine/silu/exp/add/mul/div), reduce (sum/max) shader work landed in the WGSL compute backend (`candle-core/src/vulkan_backend_wgpu/`, wgpu 22) after the 0.11.0 sync pruned the old `candle-core/shaders/` GLSL→SPIR-V (naga) set; dequant-to-F16 wedge defers fused Q-kernels. | ✅ Completed |
