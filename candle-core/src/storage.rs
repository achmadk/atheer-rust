...
impl Storage {
    ...
    pub(crate) fn same_device(&self, rhs: &Self, op: &'static str) -> Result<()> {
        ...
        // Metal requires the exact device (not just the same location) because distinct
        // MetalDevice objects on the same GPU use separate command queues. Vulkan, like
        // CUDA, uses a single logical device/queue per GPU on Android, so a location
        // comparison is sufficient (and generalizes better to future multi-GPU setups).
        // See design.md decision in proposal `add-vulkan-device-backend`.
